...
#define BOND_DENSITY(LAT) ({                \
    int sum = 0;                               \
    for (int i = 0; i < LATICE_SIZE; i++) { \
        int next = (i+1) % LATICE_SIZE;        \
        sum+=2*abs(LAT[i]-LAT[next]);    \
    }                                          \
...
__global__ void generate_kernel(
    ...
    __shared__ float W0;
    __shared__ int latice_update_counter_iter;

    // __shared__ int sum_latice;
    ...
    LATICE[threadIdx.x] = threadIdx.x&1;
    while (monte_carlo_steps < MAX_MCS) {
        ...
        if (first_i <= threadIdx.x && threadIdx.x <= last_i) {
            ...
            atomicAdd(&latice_update_counter_iter,1);
        }
        __syncthreads();
        if (latice_update_counter_iter == 0) {
            LATICE[threadIdx.x] = NEXT_STEP_LATICE[threadIdx.x];
            __syncthreads();
            for (int i = LATICE_SIZE/2; i != 0; i /= 2) {
                if (threadIdx.x < i) {
                    // Use NEXT_STEP_LATICE as cache array
                    NEXT_STEP_LATICE[threadIdx.x] += NEXT_STEP_LATICE[(threadIdx.x+i)];
                }
                __syncthreads();
            }
            if (threadIdx.x == 0) {
                if ( NEXT_STEP_LATICE[0] == 0 || NEXT_STEP_LATICE[0] == LATICE_SIZE ) {
    ...
    NEXT_STEP_LATICE[threadIdx.x] = 2*abs(LATICE[threadIdx.x]-LATICE[(threadIdx.x+1) % LATICE_SIZE]);
    __syncthreads();
    for (int i = LATICE_SIZE/2; i > 0; i /= 2) {
        if (threadIdx.x < i) {
            // Use NEXT_STEP_LATICE as cache array
    ...
    if (threadIdx.x == 0) {
        DEV_BOND_DENSITY[BLOCK_ID] = ((float)NEXT_STEP_LATICE[0] / (float)(2*LATICE_SIZE));
